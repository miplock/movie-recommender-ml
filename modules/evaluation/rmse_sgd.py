# RMSE bootstrap evaluation entrypoint for SGD configuration.
"""Run bootstrap RMSE evaluation for SGD model configuration.

EN:
Builds evaluation config for SGD and delegates execution to the shared
bootstrap RMSE routine.

PL:
Buduje konfigurację ewaluacji dla SGD i deleguje wykonanie do
...
MODEL_PATH = "models_trained/model_SGD_rmse.pkl"
# Ustawiamy bazową ścieżkę zapisu predykcji dla kolejnych runów.
PRED_FILE = "results/preds_sgd_rmse.csv"
...
RESULTS_PKL = "results/sgd_rmse_runs.pkl"
...
def main() -> None:
    """Run bootstrap RMSE experiment for SGD configuration.

    EN:
    Creates `RMSEEvalConfig` for SGD and runs shared bootstrap
    evaluation routine.
    ...
    PL:
    Tworzy konfigurację `RMSEEvalConfig` dla SGD i uruchamia wspólną
